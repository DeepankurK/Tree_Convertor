...
class compare_tree:
    def __init__(self,tree1='',label1={},tree2='',label2={},m=0,z1=False,z2=False,typ=0):
        ...
        self.typ=typ
        self.m=m
        #print(tree1, label1,tree2, label2)
...
'''
    ...
    def rf(self):
        with open(path_output+"tree1.txt",'w+') as file1 :
            Phylo.write(self.obj1.tree,file1,'newick')
        with open(path_output+"tree2.txt",'w+') as file2 :
            Phylo.write(self.obj2.tree,file2,'newick')
        tns = dendropy.TaxonNamespace()
        t1 = dendropy.Tree.get(file=open(path_output+"tree1.txt", 'r'),
                schema="newick",taxon_namespace=tns)
        t2 = dendropy.Tree.get(file=open(path_output+"tree2.txt", 'r'),
                schema="newick",taxon_namespace=tns)
        t1.encode_bipartitions()
        t2.encode_bipartitions()
        
        print("RF metric : ",treecompare.symmetric_difference(t1, t2))
        print("RF metric (weighted) : ",treecompare.weighted_robinson_foulds_distance(t1, t2))
...
if r==1:
    ...
    if ch2==2:
        ...
        dot=convertor.muta_tree(content2,1,z2,gv2,a,1,path_matrix,path_output)
        dot.render(filename="Converted_Tree",directory=path_output)
